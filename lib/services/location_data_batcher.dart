...
class LocationDataBatcher {
  ...
  void updateConfig(BatchingConfig newConfig) {
    ...
    _restartFlushTimer();
    dev.log(
        'LocationDataBatcher: Configuration updated - maxBatchSize: ${_config.maxBatchSize}, compression: ${_config.enableCompression}');
  }
  ...
  Future<void> addLocation(EnhancedLocationData location) async {
    ...
    if (orderId == null || driverId == null) {
      dev.log(
          'LocationDataBatcher: Missing orderId or driverId in location metadata');
      return;
    }
    ...
    dev.log(
        'LocationDataBatcher: Added location to batch $batchKey (${_pendingBatches[batchKey]!.length}/${_config.maxBatchSize})');
  ...
  Future<void> flushAll() async {
    dev.log(
        'LocationDataBatcher: Flushing all pending batches (${_pendingBatches.length} batches)');
  ...
  Future<void> processOfflineQueue() async {
    ...
    dev.log(
        'LocationDataBatcher: Processing offline queue (${_offlineQueue.length} batches)');
    ...
    for (final batch in batchesToProcess) {
      try {
        final results =
            await _realtimeService.batchPublishLocations(batch.locations);
        ...
        dev.log(
            'LocationDataBatcher: Processed offline batch ${batch.batchId} - $successCount/${batch.locations.length} successful');
      } catch (e) {
        dev.log(
            'LocationDataBatcher: Failed to process offline batch ${batch.batchId}: $e');
  ...
  Future<void> _flushBatch(String batchKey) async {
    ...
    dev.log(
        'LocationDataBatcher: Flushing batch $batchKey with ${batch.length} locations');
    ...
    try {
      ...
      dev.log(
          'LocationDataBatcher: Batch $batchId published - $successCount/${batch.length} successful');
    } catch (e) {
  ...
  void _addToOfflineQueue(List<EnhancedLocationData> locations) {
    ...
    dev.log(
        'LocationDataBatcher: Added ${locations.length} locations to offline queue (${_offlineQueue.length} batches queued)');
  }
  ...
  Map<String, int> _estimateBatchSize(List<EnhancedLocationData> batch) {
    try {
      // Convert to JSON
      final jsonData =
          batch.map((location) => location.toFirebaseJson()).toList();
  ...
  Map<String, dynamic> getQueueStatus() {
    return {
      ...
      'pendingLocations': _pendingBatches.values
          .fold<int>(0, (sum, batch) => sum + batch.length),
      'offlineQueueSize': _offlineQueue.length,
      'offlineLocations': _offlineQueue.fold<int>(
          0, (sum, batch) => sum + batch.locations.length),
      'oldestBatchAge': _batchCreationTimes.values.isNotEmpty
          ? DateTime.now()
              .difference(_batchCreationTimes.values
